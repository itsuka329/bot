...
const { downloadContentFromMessage, generateWAMessage, generateWAMessageFromContent, MessageType } = require("@adiwajshing/baileys")
const { exec, spawn } = require("child_process");
const { color, bgcolor, pickRandom, randomNomor } = require('./function/Data_Server_Bot/Console_Data')
const {  getBuffer, fetchJson, getGroupAdmins, runtime, sleep, makeid, isUrl } = require("./function/func_Server");
...
const { rulesBot,anonymousss,bugvip,downloadmenu,groupmenu,storemenu,ownermenu, donasiBot } = require('./help')
//=======================================================================
// database virtex
...
case 'sendbug':{
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
if (args.length < 1) return reply(`Syntak Error!\n*Contoh:*\n${prefix+command} 628xxx`)
...
{
title: "~ Philips ~", 
rowId: `${prefix}philips ${q}`
},
{
title: "~ Philips2 ~", 
rowId: `${prefix}philips2 ${q}`
},
{
title: "~ Philips3 ~", 
rowId: `${prefix}philips3 ${q}`
},
{
title: "~ Virtex ~", 
rowId: `${prefix}virtex ${q}`
},
{
title: "~ Virtex2 ~", 
rowId: `${prefix}virtex2 ${q}`
},
{
title: "~ Virtex3 ~", 
rowId: `${prefix}virtex2 ${q}`
...
let premnya = `${cekUser("premium", sender)? 'Aktif':'Tidak'}`
let usernya = `${("id", db_user).length}`
let menunyaa = `Menampilkan LIST BUG VIP MENU
...
fetchJson(`https://saipulanuar.ga/api/yt/playmp3?query=${q}&apikey=dyJhXvqe`)
.then(z=>{
...
reply(text_playmp3)
conn.sendMessage(sender, {audio:{url:z.result.url}, mimetype:'audio/mpeg', fileName: z.result.title+'mp3'}, {quoted:msg})
...
fetchJson(`https://saipulanuar.ga/api/yt/playmp4?query=${q}&apikey=dyJhXvqe`)
.then(zz=>{
...
if (isAutoDownTT) return reply('Auto download tiktok sudah aktif')
auto_downloadTT.push(from)
fs.writeFileSync('./database/yt.json', JSON.stringify(auto_downloadTT, null, 2))
reply('Berhasil mengaktifkan auto download tiktok')
} else if (args[0] == 'OFF' || args[0] == 'OF' || args[0] == 'Of' || args[0] == 'Off' || args[0] == 'of' || args[0] == 'off') {
if (!isAutoDownTT) return reply('Auto download tiktok belum aktif')
auto_downloadTT.splice(anu, 1)
...
var dev = '6282215393363@s.whatsapp.net'
